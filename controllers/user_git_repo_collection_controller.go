...
func (ctrl *UserGitRepoCollectionController) Init(ctx *core.APPContext, router *gin.RouterGroup) {
	...
	collections := router.Group("/collections")
	{
		...
		collections.GET("/repo/:repoId/:collectionName/files/resolve-edit-path", ctrl.ResolveEditPath)
...
type ResolveEditPathResponse struct {
...
func (ctrl *UserGitRepoCollectionController) ResolveEditPath(c *gin.Context) {
	reqParam := core.NewRequestParam()
	userId := reqParam.AddContextParam("userId", false, nil).
		SetError(http.StatusUnauthorized, "Unauthorized")
	repoIDParam := reqParam.AddUrlParam("repoId", false, regexp.MustCompile(`\d+`))
	...
	resolvedPath, changed, err := ctrl.service.ResolveCollectionEditPath(repo, collectionName.String(), pathParam.String())
	if err != nil {
		log.Errorf("Failed to resolve edit path: %v", err)
		core.HandleError(c, err)
		return
	}

	c.JSON(http.StatusOK, ResolveEditPathResponse{
		Path:    resolvedPath,
		Changed: changed,
	})
}
